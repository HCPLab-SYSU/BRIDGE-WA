...
class BaseActionSpace(nn.Module):
    ...
    def compute_loss_raw(self, pred: torch.Tensor, target: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
        Unscaled component losses for logging. Defaults to compute_loss.
        Subclasses should override if compute_loss applies scaling factors.
        """
        return self.compute_loss(pred, target)
...
@register_action("ee6d_abs")
@register_action("libero_abs_ee6d")
@register_action("libero_abs_ee6d20")
class EE6DActionSpace(BaseActionSpace):
    ...
    def compute_loss(self, pred, target):
        ...
        # Gripper BCE
        g_losses = [self.bce(pred[:, :, gi], (target[:, :, gi] > 0).float()) for gi in self.gripper_idx]
    ...
    def compute_loss_raw(self, pred, target):
        assert pred.shape == target.shape, "pred/target shapes must match"
        B, T, D = pred.shape
        _ensure_indices_valid(D, self.gripper_idx, "gripper_idx")
...
@register_action("ee6d10")
@register_action("ee6d_10")
@register_action("ee6d10_abs")
@register_action("libero_abs_ee6d10")
@register_action("libero_abs_ee6d_10")
class EE6D10ActionSpace(BaseActionSpace):
    """Single-arm EE6D (xyz + rot6d + gripper)."""
    ...
    def compute_loss(self, pred, target):
        ...
        target_grip = (target[:, :, self.gripper_idx[0]] > 0).float()
        gripper_loss = self.bce(pred[:, :, self.gripper_idx[0]], target_grip) * self.GRIPPER_SCALE
    ...
    def compute_loss_raw(self, pred, target):
        ...
        target_grip = (target[:, :, self.gripper_idx[0]] > 0).float()
        gripper_loss = self.bce(pred[:, :, self.gripper_idx[0]], target_grip)
        pos_loss = self.mse(pred[:, :, self.POS_IDX], target[:, :, self.POS_IDX])
        rot_loss = self.mse(pred[:, :, self.ROT_IDX], target[:, :, self.ROT_IDX])
        return {
            "position_loss": pos_loss,
            "rotate6D_loss": rot_loss,
            "gripper_loss": gripper_loss,
    ...
    def postprocess(self, action: torch.Tensor) -> torch.Tensor:
        """Apply sigmoid to gripper logits."""
        if action.size(-1) > self.gripper_idx[0]:
            action[..., self.gripper_idx] = torch.sigmoid(action[..., self.gripper_idx])
        return action
...
class JointActionSpace(BaseActionSpace):
    ...
    def compute_loss_raw(self, pred, target):
        ...
        joints_idx = tuple(i for i in range(D) if i not in set(self.gripper_idx))
        joints_loss = self.mse(pred[:, :, joints_idx], target[:, :, joints_idx])
        return {
            "joints_loss": joints_loss,
            "gripper_loss": gripper_loss,
        }
...
@register_action("bridge_libero_abs")
@register_action("bridge_libero")
@register_action("libero_abs")
@register_action("libero")
class LiberoActionSpace(BaseActionSpace):
    """Libero action space (7D action padded to 8D) with weighted MSE loss."""
    ...
    def compute_loss(self, pred, target):
        assert pred.shape == target.shape, "pred/target shapes must match"
        if pred.shape[-1] < 7:
            raise ValueError(f"Libero action expects >=7 dims, got {pred.shape[-1]}")
        pred7 = pred[..., :7]
        target7 = target[..., :7]
        pos_loss = self.mse(pred7[:, :, self.POS_IDX], target7[:, :, self.POS_IDX]) * self.POS_SCALE
        rot_loss = self.mse(pred7[:, :, self.ROT_IDX], target7[:, :, self.ROT_IDX]) * self.ROT_SCALE
        target_grip = (target7[:, :, self.gripper_idx] > 0).float()
        grip_loss = self.bce(pred7[:, :, self.gripper_idx], target_grip) * self.GRIPPER_SCALE
        return {
            "position_loss": pos_loss,
    ...
    def compute_loss_raw(self, pred, target):
        ...
        pred7 = pred[..., :7]
        target7 = target[..., :7]
        pos_loss = self.mse(pred7[:, :, self.POS_IDX], target7[:, :, self.POS_IDX])
        rot_loss = self.mse(pred7[:, :, self.ROT_IDX], target7[:, :, self.ROT_IDX])
        target_grip = (target7[:, :, self.gripper_idx] > 0).float()
        grip_loss = self.bce(pred7[:, :, self.gripper_idx], target_grip)
        return {
            "position_loss": pos_loss,
            "rotation_loss": rot_loss,
            "gripper_loss": grip_loss,
...
@register_action("ee6d_continuous_gripper")
@register_action("dobot_nova2_ee6d")
@register_action("dobot_ee6d_abs")
@register_action("dobot_ee6d")
class ContinuousGripperEE6DActionSpace(BaseActionSpace):
    """EE6D layout with continuous gripper channels regressed by MSE."""
    ...
    def compute_loss_raw(self, pred, target):
        ...
        gripper_loss = self.mse(pred[:, :, self.gripper_idx], target[:, :, self.gripper_idx])
        pos_loss = (
            self.mse(pred[:, :, self.POS_IDX_1], target[:, :, self.POS_IDX_1]) +
            self.mse(pred[:, :, self.POS_IDX_2], target[:, :, self.POS_IDX_2])
        )
    ...
    def preprocess(self, proprio, action, mode="train"):
        """Keep gripper proprio and noisy gripper action for continuous denoising."""
        return proprio, action

    def postprocess(self, action: torch.Tensor) -> torch.Tensor:
        """Continuous gripper commands are emitted in the training scale."""
        return action


@register_action("agibot_ee6d")
class AGIBOTEE6DActionSpace(ContinuousGripperEE6DActionSpace):
    """Backward-compatible alias for continuous-gripper EE6D."""
