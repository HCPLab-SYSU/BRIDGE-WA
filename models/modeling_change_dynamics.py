...
from .configuration_change_dynamics import ChangeDynamicsConfig
...
class ChangeMapProjector(nn.Module):
    def __init__(
        self,
        out_dim: int,
        token_count: int = 16,
        ...
        if pool_hw * pool_hw != token_count:
            raise ValueError("change_token_count must equal change_map_pool_hw ** 2.")
    ...
    def forward(self, change_map: torch.Tensor) -> torch.Tensor:
        x = change_map.float()
        if x.dim() == 3:
            x = x.unsqueeze(1)
        if x.dim() != 4 or x.shape[1] != 1:
            raise ValueError(f"Expected change map [B,1,H,W] or [B,H,W], got {tuple(change_map.shape)}")
...
class ChangeMapPredictor(nn.Module):
    def __init__(
        self,
        input_dim: int,
        hidden_size: int,
        ...
        if token_dim % num_heads != 0:
            raise ValueError("change token_dim must be divisible by change_predictor_num_heads.")
        self.token_count = token_count
        self.pool_hw = pool_hw
        ...
        self.out_proj = nn.Linear(token_dim, 1)
    ...
    def forward(self, vlm_features: torch.Tensor, aux_visual_inputs: torch.Tensor) -> torch.Tensor:
        ...
        return x.transpose(1, 2).reshape(vlm_features.shape[0], 1, self.pool_hw, self.pool_hw)


class ChangeConditionedAttention(nn.Module):
    def __init__(
        self,
        dim: int,
        num_heads: int = 8,
    ...
    def forward(self, x: torch.Tensor, change_tokens: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if change_tokens is not None and change_tokens.numel() > 0:
            ...
            if self.gate is not None:
                ...
                cond = cond * gate
            k_change = self._reshape(self.k_change(cond))
            v_change = self._reshape(self.v_change(cond))
            k = torch.cat([k, k_change], dim=2)
            v = torch.cat([v, v_change], dim=2)
...
class ChangeTransformerBlock(nn.Module):
    def __init__(
        self,
        hidden_size: int,
        num_heads: int,
        ...
        if inject_change:
            self.attn = ChangeConditionedAttention(
                hidden_size,
                num_heads=num_heads,
                qkv_bias=True,
                attn_drop=0.1,
    ...
    def forward(self, x: torch.Tensor, change_tokens: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.inject_change:
            x = x + self.attn(self.norm1(x), change_tokens=change_tokens)
        else:
...
class ChangeSoftPromptedTransformer(nn.Module):
    def __init__(
        self,
        hidden_size: int = 768,
        ...
        if change_injection_start_layer is None:
            change_injection_start_layer = max(0, depth - change_num_injected_layers)
        end_layer = min(depth, change_injection_start_layer + change_num_injected_layers)
        self.change_injected_layers = set(range(max(0, change_injection_start_layer), end_layer))

        self.blocks = nn.ModuleList(
            [
                ChangeTransformerBlock(
                    hidden_size,
                    num_heads,
                    mlp_ratio=mlp_ratio,
                    inject_change=(idx in self.change_injected_layers),
                    use_gate=change_use_gate,
                )
    ...
    def forward(
        ...
        for idx, block in enumerate(self.blocks):
            block_change = change_tokens if idx in self.change_injected_layers else None
            x = block(x, change_tokens=block_change)

        return self.action_decoder(self.norm(x[:, :num_actions]), domain_id)


class ChangeDynamics(PreTrainedModel):
    config_class = ChangeDynamicsConfig
    base_model_prefix = "change_dynamics"
    supports_gradient_checkpointing = True

    def __init__(self, config: ChangeDynamicsConfig, *args, **kwargs):
        ...
        self.transformer = ChangeSoftPromptedTransformer(
            ...
            change_num_injected_layers=config.change_num_injected_layers,
            change_injection_start_layer=config.change_injection_start_layer,
            change_use_gate=config.change_use_gate,
        )
    ...
    @classmethod
    def from_vision_action_pretrained(cls, pretrained_model_name_or_path: str, **config_overrides) -> "ChangeDynamics":
        base_cfg = VisionActionConfig.from_pretrained(pretrained_model_name_or_path)
        cfg = ChangeDynamicsConfig.from_vision_action_config(base_cfg, **config_overrides)
        model = cls(cfg)
        ...
        if missing:
            print("[ChangeDynamics] Missing keys while loading VisionAction base:", missing[:12], "..." if len(missing) > 12 else "")
        if unexpected:
            print("[ChangeDynamics] Unexpected keys while loading VisionAction base:", unexpected[:12], "..." if len(unexpected) > 12 else "")
        return model
    ...
    def forward(
        ...
        if change_map is not None:
            teacher_change_lowres = F.adaptive_avg_pool2d(change_map.float(), (self.config.change_map_pool_hw, self.config.change_map_pool_hw))
        ...
        if teacher_change_lowres is not None:
            mse = F.mse_loss(predicted_change_map.float(), teacher_change_lowres.float())
            ...
            outputs["change_map_distill_loss"] = (
                float(self.config.change_distill_weight) * mse
                + float(self.config.change_distill_cosine_weight) * cos_loss
            )
            outputs["teacher_change_map_lowres"] = teacher_change_lowres
            outputs["change_map_mse"] = mse.detach()
            outputs["change_map_cosine"] = cosine.detach()
        return outputs
