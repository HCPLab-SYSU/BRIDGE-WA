...
conda run -n vlabench python -m pip install -r evaluation/vlabench/requirements-eval.txt
conda run -n vlabench python -m pip install --no-deps -e evaluation/vlabench/VLABench
conda run -n vlabench python evaluation/vlabench/check_vlabench_env.py
bash scripts/download_vlabench_assets.sh
```

Use `evaluation/vlabench/requirements-eval.txt` for policy evaluation. The
vendored VLABench `requirements.txt` also installs its training, dataset
conversion, notebook, and alternate-policy dependencies, including Git-based
LeRobot and scripted RRT packages that are not needed by the Bridge-WA client.

To repair an existing partial `vlabench` environment, rerun the two `pip`
commands above. They install the tested simulator pins and the missing imports
that VLABench does not declare in `setup.py`.
